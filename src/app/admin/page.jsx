...
function Admin() {
  ...
  useEffect(() => {
    if (userDetail && userDetail[0] && !isPaymentChecked) {
      checkPayment();
    }
  }, [userDetail, isPaymentChecked, checkedCount]);
  ...
  const checkPayment = async () => {
    ...
    let packetType = userDetail[0]?.packetType;
    if (!packetType) {
      ...
      if (packetType === "1-year") {
        ...
        if (date.isBefore(now)) {
          ...
          try {
            ...
            const response = await axios.put(`${APP_URL}/user`, {
              email,
              payment,
            });
          } catch (error) {
